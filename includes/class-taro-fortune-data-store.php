...
class YesOrNo_Data_Store {
    ...
    private function build_test_payload($post_id) {
        ...
        return array(
            ...
            // [추가됨] 프론트엔드로 전달할 커스텀 데이터
            'custom_css' => isset($basic['custom_css']) ? (string) $basic['custom_css'] : '',
            'html_template' => isset($basic['html_template']) ? (string) $basic['html_template'] : '',
    ...
    private function normalize_test_payload($payload) {
        ...
        return array(
            ...
            'basic' => array(
                ...
                // [추가됨] DB 저장을 위한 정규화 (관리자 전용이므로 HTML/CSS 원본 유지)
                'custom_css' => isset($payload['custom_css']) ? (string) $payload['custom_css'] : '',
                'html_template' => isset($payload['html_template']) ? (string) $payload['html_template'] : '',
